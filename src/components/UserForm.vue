<template>
    <Dialog v-model:visible="visible" :modal="true" :header="isEdit ? 'Editar usuario' : 'Nuevo usuario'"
        :style="{ width: '500px' }">
        <Form @submit="handleSubmit" :validation-schema="schema" :initial-values="initialValues">
            ...
            <div class="mb-4" v-if="!isEdit">
                ...
                <Field name="password" v-slot="{ field, handleChange }">
                    <Password v-model="field.value" toggleMask class="w-full" inputClass="w-full"
            ...
            <div class="mb-6">
                ...
                <Field name="role" v-slot="{ field, meta, handleChange }">
                    <Dropdown v-model="field.value" :options="roles" optionLabel="label" optionValue="value"
                        class="w-full" @change="handleChange($event.value)" />
                </Field>
